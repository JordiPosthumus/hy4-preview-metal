...
int main() {
    ...
    const size_t wq_bytes = ggml_row_size(GGML_TYPE_Q1_0, n_per_row) * (size_t)n_rows;
    std::vector<block_q1_0> wq(wq_bytes / sizeof(block_q1_0));
    printf("quantizing %d x %d ...\n", n_rows, n_per_row);
    quantize_q1_0(w.data(), wq.data(), n_rows, n_per_row, nullptr);
