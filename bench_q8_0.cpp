...
int main() {
    ...
    const size_t wq_bytes = ggml_row_size(GGML_TYPE_Q8_0, n_per_row) * (size_t)n_rows;
    std::vector<block_q8_0> wq(wq_bytes / sizeof(block_q8_0));
    printf("quantizing %d x %d ...\n", n_rows, n_per_row);
    quantize_q8_0(w.data(), wq.data(), n_rows, n_per_row, nullptr);
    ...
    ggml_tensor * A = ggml_new_tensor_2d(ctx, GGML_TYPE_Q8_0, n_per_row, n_rows);
